...
#     def forward(self, features: List[torch.Tensor]) -> torch.Tensor:
#         print(f"🚀 UnetDecoder received: {type(features)} with length: {len(features) if isinstance(features, list) else 'N/A'}")

#         # 🚨 Fix the error: Use `len(features) == 0` instead of `if not features`
#         if isinstance(features, list) and len(features) == 0:
#             raise ValueError("🚨 UnetDecoder received an empty feature list!")

#         if isinstance(features, torch.Tensor):
#             raise TypeError(f"🚨 Expected a list of feature maps, but got a tensor with shape: {features.shape}")
...
#         print(f"✅ Reversed feature list length: {len(features)}")  # Debug print
